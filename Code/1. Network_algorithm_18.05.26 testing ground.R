...
saveRDS(gnetwork_new,
        paste0(Key, "Output/PN_links_", product4d, ".rds"))
...
# For each disconnected node, check what Hidden_net says about it
disconnected_nodes <- Look_up

for (node in disconnected_nodes) {
  
  # What does this node feed INTO in Hidden_net? (downstream)
  downstream <- Hidden_net$row[Hidden_net$col == node]
  
  # What feeds INTO this node in Hidden_net? (upstream)
  upstream <- Hidden_net$col[Hidden_net$row == node]
  
  cat("\nNode:", node, "\n")
  cat("  Feeds into (downstream):", 
      if(length(downstream) == 0) "NONE" else downstream, "\n")
  cat("  Fed by (upstream):", 
      if(length(upstream) == 0) "NONE" else upstream, "\n")
}
