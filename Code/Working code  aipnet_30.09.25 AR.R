...
disp <- build_hierarchical_graph(Hidden_network_stage12, root = product4d, max=3)
...
# Four digit product code for every vertex
v_code <- if (!is.null(V(g)$code)) V(g)$code else substr(V(g)$name, 1, 4)

# Keep only level 4 rows from hsnames and prepare for join
hs4 <- hsnames %>%
  filter(Level == "4") %>%
  distinct(Code, .keep_all = TRUE) %>%
  transmute(code = Code, code_desc = Description)

# Build edges and nodes for visNetwork
...
nodes <- data.frame(
  ...
  left_join(hs4, by = c("code" = "code"))

# Make a readable label that includes code and its name when available
nodes$label <- ifelse(
  is.na(nodes$code_desc),
  nodes$label,
  paste0(nodes$label, "\n[", nodes$code, "] ", nodes$code_desc)
)

# Simple HTML escaping for tooltip text
...
nodes$title <- sprintf(
  "<b>%s</b><br>Code: %s%s<br>Stage: %s<br>Level: %s<br>Branch: %s",
  nodes$id,
  nodes$code,
  ifelse(is.na(nodes$code_desc), "", paste0("<br>", escape_html(nodes$code_desc))),
  nodes$stage, nodes$level, nodes$branch_id
)

# Keep label text if you want it later, but hide visible labels now
nodes$hidden_label <- nodes$label
nodes$label <- ""

# Optional grouping by stage for colour
