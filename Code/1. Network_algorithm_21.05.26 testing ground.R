...
# ── Restore products lost due to kuj2 fix ─────────────────────────────────────
# The kuj[y] → kuj2[y] fix correctly computes Upstream1 scores but as a
# side effect stops including anchor nodes in Full_list. This means purely
# upstream products — those that never appear as a downstream target in any
# chain — are no longer accidentally rescued by the bug and go missing from
# all_nodes. We restore them explicitly here by ensuring every product in
# potential_p1 is included as a vertex, which is what the bug was
# accidentally guaranteeing before.
all_nodes <- bind_rows(
  all_nodes,
