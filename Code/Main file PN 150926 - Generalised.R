...
PN_networkmodel<-function(product_code,Agreg_level,AIPNET_data,hsnames,BEC_database, Local_path){
...
if (length(nodes_without_edges) > 0) {
  ...
  missing_edges <- graphh_clean[
    graphh_clean$hs2002_code_upstream   %in% nodes_without_edges &
      graphh_clean$hs2002_code_downstream %in% potential_p1, ] %>%
    transmute(
      from   = hs2002_code_upstream,
