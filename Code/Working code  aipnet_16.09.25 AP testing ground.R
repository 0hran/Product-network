...
exclude_capital_goods <- function(graphh_clean, hsnames, product4d) {
  # Prepare HS names at 4-digit level
  hsnames1 <- hsnames %>%
    filter(Level == "4") %>%
    select(Code, Description) %>%
    rename(code = Code, description = Description)
  
  # Exclude 84xx, 85xx, except the selected root if it falls in those chapters
  exclude_vec <- hsnames1 %>%
    filter(str_detect(code, "^(84|85)") & code != product4d) %>%
    pull(code)
  
  graphh_clean %>%
    filter(!hs2022_code_upstream   %in% exclude_vec,
           !hs2022_code_downstream %in% exclude_vec)
}
...
t1<-graphh_clean[graphh_clean$hs2022_code_downstream%in%potential_p1,] #Total input list of the product 0 inputs
...
Hidden_network_stage1<-Hidden_network[!Hidden_network$hs2022_code_upstream%in%t1$hs2022_code_downstream,]
...
t2<-cbind.data.frame(hs2022_code_upstream =unique(t1$hs2022_code_downstream[!t1$hs2022_code_downstream%in%t1$hs2022_code_upstream]) ,hs2022_code_downstream =product4d)
Hidden_network_stage1<-rbind.data.frame(Hidden_network_stage1, t2)

Hidden_network_stage12<-rbind.data.frame(Hidden_network_stage1, t1)
...
disp <- build_hierarchical_graph(Hidden_network_stage12, root = root_code, max=3)
...
list_of_target_node<-V(display_network)$name[V(display_network)$level==3]
...
length(unique(c(Hidden_network$hs2022_code_upstream,Hidden_network$hs2022_code_downstream, t1$hs2022_code_upstream, t1$hs2022_code_downstream)))
