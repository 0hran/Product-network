...
# Import ####
  ...
  # AIPNET 6 digit
#  graphh <- read_excel("C:/Users/ap115/OneDrive - SOAS University of London/CSST Arnaud_Adria/Code/Aipnet hs22_6d.xlsx") # Arnaud
...
graphh <- read.csv("C:/Users/ap115/OneDrive - SOAS University of London/CSST Arnaud_Adria/Code/edge_list_hs2022_4digit.csv") # Arnaud
 ...
 product_codes_HS22_V202501 <- read_csv("C:/Users/ap115/OneDrive - SOAS University of London/CSST Arnaud_Adria/Code/product_codes_HS22_V202501.csv")
...
category <- read_excel("C:/Users/ap115/OneDrive - SOAS University of London/CSST Arnaud_Adria/Code/AIPNET_Data_Pack_20241204.xlsx", sheet = "1a. Node List 6-digit HS02")
...
# Clean ####
  ...
  # Remove cycles
    ...
    # Option 1: pagerank
    # start from your original edge list and drop exact duplicates just in case
    edges <- graphh_c %>%
      distinct(hs2022_code_upstream, hs2022_code_downstream)
    
    # build directed graph and compute PageRank
    g <- graph_from_data_frame(edges, directed = TRUE)
    pr_vec <- page_rank(g, directed = TRUE)$vector
    
    pr_tbl <- tibble(
      hs2022_code = names(pr_vec),
      pagerank = as.numeric(pr_vec)
    )
    
    # attach PageRank to both ends of every edge
    edges_scored <- edges %>%
      left_join(pr_tbl, by = c("hs2022_code_upstream" = "hs2022_code")) %>%
      rename(pr_up = pagerank) %>%
      left_join(pr_tbl, by = c("hs2022_code_downstream" = "hs2022_code")) %>%
      rename(pr_down = pagerank) %>%
      # make an order free pair id so A->B and B->A group together
      mutate(pair_id = paste(
        pmin(hs2022_code_upstream, hs2022_code_downstream),
        pmax(hs2022_code_upstream, hs2022_code_downstream),
        sep = "::"
      ))
    
    # for each pair:
    # - if only one direction exists, keep it
    # - if both directions exist, keep the one where upstream has the higher PageRank
    #   in an exact tie keep a single arrow deterministically by sorting codes
    graphh2 <- edges_scored %>%
      group_by(pair_id) %>%
      # choose one row per pair
      slice({
        if (n() == 1) {
          1L
        } else {
          # prefer the larger value of (pr_up - pr_down)
          diffs <- pr_up - pr_down
          winners <- which(diffs == max(diffs, na.rm = TRUE))
          # if this is a tie choose deterministically by lexicographic order of the
          # concatenated arrow "upstream->downstream"
          if (length(winners) > 1) {
            arrows <- paste(hs2022_code_upstream, hs2022_code_downstream, sep = "->")
            winners[which.min(arrows[winners])]
          } else {
            winners
          }
        }
      ...
      ungroup() %>%
      select(hs2022_code_upstream, hs2022_code_downstream)
    
    # graphh2 is your cleaned network
    str(graphh2)
...
# Create network ####
  ...
  # 4 digits
    ...
    network_4d<-build_full_network(product4d,graph_clean) # change network object name accorindgly
    #product_codes_HS22_V202501
...
# Draw plot ####
  ...
  tg <- as_tbl_graph(g)
  tg <- tg %>%
    mutate(label = ifelse(name == "8703", "8703", NA),
           highlight = name == "8703")
...
# Explore if we can stratify into hierarchy using centrality etc. measures
  ...
  nodes_list<-as.data.frame(vertex.attributes(g_prod))


  write.csv(as.matrix(get.adjacency(g_prod)),"C:/Users/ap115/OneDrive - SOAS University of London/R/AS.csv", row.names = T)
   colnames(nodes_list)<-c("id","centrality","label")

   write.csv(nodes_list,"C:/Users/ap115/OneDrive - SOAS University of London/R/nodes_list.csv", row.names = FALSE)
